...
export const initiateProfile = async (req, res, next) => {
    try {
...
export const getProfileComments = async (req, res, next) => {
    try {
        ...
        const comments = await knexInstance('profile_comments')
            .select('profile_comments.id', 'profile_comments.content', 'profile_comments.created_at', 'users.firebase_uid as fromUserId', 'users.username as fromUsername')
...
export const postProfileComment = async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
...
export const vouchUser = async (req, res, next) => {
    try {
        const { firebase_uid } = req.params;
